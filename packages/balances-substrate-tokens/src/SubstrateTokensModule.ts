...
import { ChainId, ChaindataProvider, NewTokenType, SubChainId, githubTokenLogoUrl } from "@talismn/chaindata-provider"
import { filterMetadataPalletsAndItems, metadataIsV14, mutateMetadata } from "@talismn/mutate-metadata"
...
> = (hydrate) => {
  ...
  return {
    ...
    async transferToken({
      ...
      const tokensArgs = sendAll ? { dest: to, currencyId, keepAlive: false } : { dest: to, currencyId, amount }
...
async function buildQueries(
  ...
  return Object.entries(addressesByToken).flatMap(([tokenId, addresses]) => {
    ...
    if (token.type !== "substrate-tokens") {
      console.debug(`This module doesn't handle tokens of type ${token.type}`)
      return []
    }

    const chainId = token.chain?.id
    if (!chainId) {
      console.warn(`Token ${tokenId} has no chain`)
      return []
    }

    const chain = chains[chainId]
    if (!chain) {
      console.warn(`Chain ${chainId} for token ${tokenId} not found`)
      return []
    }

    const chainMeta = findChainMeta<typeof SubTokensModule>("substrate-tokens", chain)
    const registry =
      chainMeta?.metadata !== undefined && chainMeta?.metadata !== null && chainMeta?.metadataVersion >= 14
